...
final class DetectionEngine: ObservableObject {
    ...
    struct RFFeatureVector {
        ...
        let hrStd: Float
        let lfHfRatio: Float
        let pnn50: Float    // % of successive RR pairs differing by >50ms (from HRVExtractor)
    ...
    private func buildFeatureVector() -> RFFeatureVector {
        ...
        return RFFeatureVector(
            ...
            hrStd: hrStd,
            lfHfRatio: lfHfRatio,
            pnn50: Float(hrvExtractor.currentPNN50),
            ageGroup: ageGroup,
            timeOfDay: timeOfDay,
    ...
    /// The PanicGuardRF.mlpackage has 9 named scalar Double inputs and two outputs:
    ///   - panicProbability: Float (0-1) direct panic class probability
    ///   - classLabel: Int64 predicted class (0=baseline, 1=stress_response, 2=panic_onset)
    private func runRFInference(rfModel: PanicGuardRF, featureVector: RFFeatureVector) -> Double {
        do {
            // PanicGuardRF.mlpackage exports 9 named Double inputs (not MLMultiArray).
            // CoreML Swift infers keyword argument names from the model input names.
            let output = try rfModel.prediction(
                rmssd: Double(featureVector.rmssd),
                sdnn: Double(featureVector.sdnn),
                hr_mean: Double(featureVector.hrMean),
                hr_std: Double(featureVector.hrStd),
                lf_hf_ratio: Double(featureVector.lfHfRatio),
                pnn50: Double(featureVector.pnn50),
                age_group: Double(featureVector.ageGroup),
                time_of_day: Double(featureVector.timeOfDay),
                sleep_hours: Double(featureVector.sleepHours)
            )

            // Read panic probability directly from the panicProbability output.
            // The export script outputs a single panicProbability Float (0-1),
            // NOT a classProbability dictionary. classLabel is also available.
            return output.panicProbability
